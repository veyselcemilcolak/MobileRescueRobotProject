...
def obstacle_body_frame(ir_id, ir_value):
    ...
    dist =  -1 / 19 * math.log((ir_value+1) / 3800)
...
def vel_control(robot_loc, target_loc, IR_obs):
    ...
    move_vector = Vec2(target_loc[0] - robot_loc[0], target_loc[1] - robot_loc[1])
    dist_target = move_vector.norm()    # distance vector between target and current position
    ...
    trans_speed = min(1.0, max(0.5*dist_target*move_vector.norm(), 0.1))

    if move_vector.norm() < TOLERANCE:
        trans_speed = 0.0
        rot_speed = 0.0
